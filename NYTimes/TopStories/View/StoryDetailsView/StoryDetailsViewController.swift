...
class StoryDetailsViewController: UIViewController, ViewAlertProtocol {
    ...
    @IBAction func bookmarkButtonTapped(_ sender: Any) {
        ...
        if(!isBookmarked) {
            if(CoreDataStack.sharedInstance.addToBookmarks(story: currentStory)) {
                alert.title = "Added"
                alert.message = "Story bookmarked"
                setBookmarkButton()
            }
            else {
                //Todo: return an error instead of bool
            }
        }
        else {
            if(CoreDataStack.sharedInstance.removeBookmark(story: currentStory)) {
                alert.title = "Removed"
